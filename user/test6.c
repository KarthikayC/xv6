...
test_pa4_18(void)
{
    ...
    setraidpolicy(0); // RAID0 for simplicity
...
test_pa4_19(void)
{
    ...
    setraidpolicy(0); // RAID0 — simplest path
    int pid = getpid2();
...
test_pa4_20(void)
{
    ...
    for (int c = 0; c < NCHILDREN_20; c++) {
        ...
        done_20:;
            ...
            rep.disk_reads       = st.disk_reads;
            rep.disk_writes      = st.disk_writes;
    ...
    for (int c = 0; c < NCHILDREN_20; c++) {
        ...
        printf("  child %d (%s): pid=%d reads=%d writes=%d"
               ...
               c, mnames[c], r->pid,
               r->disk_reads, r->disk_writes,
               r->avg_disk_latency/100, r->avg_disk_latency%100,
               r->errors);
